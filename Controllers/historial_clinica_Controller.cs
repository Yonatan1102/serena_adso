...
namespace WebApplication1.Controllers
{
    ...
    {
        private readonly Ihistorial_clinico historialClinico_Repositories;

        public historial_clinico_controller(Ihistorial_clinico historialClinico_Repositories)
        {
            this.historialClinico_Repositories = historialClinico_Repositories;
        }
        [HttpGet]
        public async Task<IActionResult> ListarHistorialClinicos()
        {
            try
            {
                var response = await historialClinico_Repositories.Gethistorial_clinico();
                return Ok(response);
            }
        ...
        public async Task<IActionResult> ObtenerHistorialClinico(int id)
        {
            try
            {
                var response = await historialClinico_Repositories.Gethistorial_clinicoById(id);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> crear_historial_clinico([FromBody] historial_clinico historial_clinico)
        {
            try
            {
                ...
                var response = await historial_cita_repositories.Posthistorial_clinico(historial_clinico);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> CrearHistorialClinico([FromBody] historial_clinico historialClinico)
        {
            try
            {
                var response = await historialClinico_Repositories.Posthistorial_clinico(historialClinico);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ActualizarHistorialClinico([FromBody] historial_clinico historialClinico)
        {
            ...
            {
                var response = await historialClinico_Repositories.Puthistorial_clinico(historialClinico);
                return Ok(response);
            }
            catch (Exception ex)
            {
