...
public class estado_de_animo_repositories : Iestado_de_animo
{
    ...
    public async Task<estado_de_animo?> Putestado_de_animo(estado_de_animo value){var item=await context.estado_de_animo.FindAsync(value.id_estado);if(item==null)return null;item.nombre_estado=value.nombre_estado;item.fecha_estado=value.fecha_estado;item.id_usuario=value.id_usuario;await context.SaveChangesAsync();return item;}
    public async Task<bool> Deleteestado_de_animo(int id){var item=await context.estado_de_animo.FindAsync(id);if(item==null)return false;context.estado_de_animo.Remove(item);await context.SaveChangesAsync();return true;}
}
